...
class SaveTransferUseCase(private val transferRepository: TransferRepository) {
    suspend fun execute(transfer: TransferRoom) = transferRepository.saveTransfer(transfer)
}